...
export function renderProducts(products, currentPage, productsPerPage, productGrid, currentProducts) {
    ...
    paginatedProducts.forEach((product) => {
        ...
        productCard.innerHTML = `
            ...
            <button aria-label="зүрх" class="heart-button" data-id="${product.id}">
    ...
    updatePagination(products.length, currentPage, productsPerPage, currentProducts);
}
...
export function filterProducts() {
    ...
    const filterKeywords = new Map(
        Array.from(keywordDisplay.querySelectorAll(".keyword-tag"))
            .map((tag) => [tag.textContent.trim().replace("×", "").trim(), true])
    );
    ...
    const filteredProducts = products.filter((product) => {
        ...
        const matchesKeywords =
            ...
            product.tag.some((prodKeyword) =>
                filterKeywords.has(prodKeyword.toLowerCase())
            );
    ...
    renderProducts(filteredProducts, currentPage, productsPerPage, productGrid, currentProducts);
...
function updatePagination(totalProducts, currentPage, productsPerPage, currentProducts) {
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        pageButton.addEventListener("click", () => {
            currentPage = i;
            renderProducts(currentProducts, currentPage, productsPerPage, productGrid, currentProducts);
        });
